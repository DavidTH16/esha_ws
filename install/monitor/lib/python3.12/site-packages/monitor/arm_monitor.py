...
class ROSMonitor_arm_monitor(Node):


	def callbackarm_current_talker(self,data):
		self.get_logger().info("monitor has observed "+ str(data))
		dict= rosidl_runtime_py.message_to_ordereddict(data)
		dict['topic']='arm_current_talker'
	...
	def __init__(self,monitor_name,log,actions):
		...
		self.topics_info['arm_current_talker']={'package': 'interfaces.msg', 'type': 'CurrentVelocityData'}
		self.config_subscribers['arm_current_talker']=self.create_subscription(topic='arm_current_talker',msg_type=CurrentVelocityData,callback=self.callbackarm_current_talker,qos_profile=1000)
...
def main(args=None):
	...
	log = './log_arm_monitor.txt'
	actions = {}
	actions['arm_current_talker']=('log',0)
	monitor = ROSMonitor_arm_monitor('arm_monitor',log,actions)
	rclpy.spin(monitor)
	monitor.ws.close()
