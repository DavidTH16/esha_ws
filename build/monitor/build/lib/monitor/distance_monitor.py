...
class ROSMonitor_distance_monitor(Node):


	def callbackultrasonic_talker(self,data):
		self.get_logger().info("monitor has observed "+ str(data))
		dict= rosidl_runtime_py.message_to_ordereddict(data)
		dict['topic']='ultrasonic_talker'
	...
	def __init__(self,monitor_name,log,actions):
		...
		self.topics_info['ultrasonic_talker']={'package': 'interfaces.msg', 'type': 'SensorData'}
		self.config_subscribers['ultrasonic_talker']=self.create_subscription(topic='ultrasonic_talker',msg_type=SensorData,callback=self.callbackultrasonic_talker,qos_profile=1000)
...
def main(args=None):
	...
	log = './log_distance_monitor.txt'
	actions = {}
	actions['ultrasonic_talker']=('log',0)
	monitor = ROSMonitor_distance_monitor('distance_monitor',log,actions)
	rclpy.spin(monitor)
	monitor.ws.close()
