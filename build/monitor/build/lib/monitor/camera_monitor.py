...
class ROSMonitor_camera_monitor(Node):
	...
	def __init__(self,monitor_name,log,actions):
		...
		self.topics_info['camera_talker']={'package': 'std_msgs.msg', 'type': 'Float32'}
		self.config_subscribers['camera_talker']=self.create_subscription(topic='camera_talker',msg_type=Float32,callback=self.callbackcamera_talker,qos_profile=1000)
...
def main(args=None):
	...
	log = './log_camera_monitor.txt'
	actions = {}
	actions['camera_talker']=('log',0)
	monitor = ROSMonitor_camera_monitor('camera_monitor',log,actions)
	rclpy.spin(monitor)
	monitor.ws.close()
