...
class ROSMonitor_temperature_monitor(Node):


	def callbacktemperature_talker(self,data):
		self.get_logger().info("monitor has observed "+ str(data))
		dict= rosidl_runtime_py.message_to_ordereddict(data)
		dict['topic']='temperature_talker'
	...
	def __init__(self,monitor_name,log,actions):
		...
		self.topics_info['temperature_talker']={'package': 'interfaces.msg', 'type': 'SensorData'}
		self.config_subscribers['temperature_talker']=self.create_subscription(topic='temperature_talker',msg_type=SensorData,callback=self.callbacktemperature_talker,qos_profile=1000)
...
def main(args=None):
	...
	actions = {}
	actions['temperature_talker']=('log',0)
	monitor = ROSMonitor_temperature_monitor('temperature_monitor',log,actions)
	rclpy.spin(monitor)
	monitor.ws.close()
