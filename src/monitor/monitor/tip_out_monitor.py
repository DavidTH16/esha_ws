...
class ROSMonitor_tip_out_monitor(Node):
	...
	def __init__(self,monitor_name,log,actions):
		...
		self.topics_info['imu_talker']={'package': 'interfaces.msg', 'type': 'ImuData'}
		self.config_subscribers['imu_talker']=self.create_subscription(topic='imu_talker',msg_type=ImuData,callback=self.callbackimu_talker,qos_profile=1000)
...
def main(args=None):
	...
	log = './log_tip_out_monitor.txt'
	actions = {}
	actions['imu_talker']=('log',0)
	monitor = ROSMonitor_tip_out_monitor('tip_out_monitor',log,actions)
	rclpy.spin(monitor)
	monitor.ws.close()
