...
class ROSMonitor_motor_monitor(Node):


	def callbackmotor_current_talker(self,data):
		self.get_logger().info("monitor has observed "+ str(data))
		dict= rosidl_runtime_py.message_to_ordereddict(data)
		dict['topic']='motor_current_talker'
	...
	def __init__(self,monitor_name,log,actions):
		...
		self.topics_info['motor_current_talker']={'package': 'interfaces.msg', 'type': 'CurrentVelocityData'}
		self.config_subscribers['motor_current_talker']=self.create_subscription(topic='motor_current_talker',msg_type=CurrentVelocityData,callback=self.callbackmotor_current_talker,qos_profile=1000)
...
def main(args=None):
	...
	log = './log_motor_monitor.txt'
	actions = {}
	actions['motor_current_talker']=('log',0)
	monitor = ROSMonitor_motor_monitor('motor_monitor',log,actions)
	rclpy.spin(monitor)
	monitor.ws.close()
