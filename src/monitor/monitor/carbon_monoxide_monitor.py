...
class ROSMonitor_carbon_monoxide_monitor(Node):
	...
	def __init__(self,monitor_name,log,actions):
		...
		self.topics_info['co_talker']={'package': 'interfaces.msg', 'type': 'SensorData'}
		self.config_subscribers['co_talker']=self.create_subscription(topic='co_talker',msg_type=SensorData,callback=self.callbackco_talker,qos_profile=1000)
...
def main(args=None):
	...
	log = './log_carbon_monitor.txt'
	actions = {}
	actions['co_talker']=('log',0)
	monitor = ROSMonitor_carbon_monoxide_monitor('carbon_monoxide_monitor',log,actions)
	rclpy.spin(monitor)
	monitor.ws.close()
